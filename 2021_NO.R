...
# Set y-axis limits
y_min <- min(data_grondwater$Grondwater..gemiddeld.gemeten..m.NAP., data_grondwater$Berekend, tot_grondwater, wat_grondwater, land_grondwater, na.rm=TRUE)
y_max <- max(data_grondwater$Grondwater..gemiddeld.gemeten..m.NAP., data_grondwater$Berekend, tot_grondwater, wat_grondwater, land_grondwater, na.rm=TRUE)
ylim <- c(y_min, y_max)

# Create line plot with adjusted y-axis limits and legend
plot(data_grondwater$Grondwater..gemiddeld.gemeten..m.NAP., type = "l", col = "red", ylim = ylim,
     xlab = "Time", ylab = "Grondwater level (m NAP)")
lines(data_grondwater$Berekend, type = "l", col = "orange")
lines(tot_grondwater,type="l",col ="brown")
lines(wat_grondwater, type="l", col="blue")
lines(land_grondwater,type="l", col="green")
legend("bottomright", legend = c("Gemeten grondwater", "Berekend grondwater", 
                                 "Tot grondwater", "Wat grondwater", "Land grondwater"), col = c("red", "orange",
                                                                                                      "brown", "blue",
                                                                                                      "green"), lty = 1)
...
nitraat_inlaat <- c("1" = 1.1, "2" = 0.9, "3" = 0.4, "4" = 0.4,
                     "5" = 0.3, "6" = 0.08, "7" = 0.12, "8" = 0.15,
                     "9" = 0.2, "10" = 0.5, "11" = 0.6, "12" = 1.2)
...
nitraat_initial_value <- 0.9 #mg/l
...
measured_values <- data.frame(date = as.Date(c("2021-01-14", "2021-02-18", "2021-03-11", "2021-04-15",
                              ...
                              NO_mg_l = c(0.977,0.663,0.728,0.09,0.033,0.014,0.012,0.038,
                                          0.01,0.373,0.813,0.954))
...
plot_NO <- ggplot(df_NO, aes(x = datum)) +
  ...
  ggtitle("NO3 concentration in the Waardassacker in 2021") +
...
file_path <- "C:/Users/Kok30/OneDrive - Waternet Amsterdam/df_NO_2021_minderinlaat.xlsx"
